...
var settingsTip = d3.tip()
    .direction('s') 
    .offset([14 ,-15])
    .attr("class","tip")
    .html(function(d) {
        return "<div id='settings' onclick='redirect_to_login()'>Login</div>";
        })
    .style("color", backColor);
    
svg.call(bookmarkTip);
svg.call(settingsTip);
...
function drawIcons() {
    ...
    //GEAR Icon
    var gear;
    var gear_back = iconG.append("circle")
            .attr("cy", iconInset * 0.48 + iconInset/2 )
            .attr("cx", (width - (iconInset * 2.7)+ iconInset/2) )
            .attr("r", iconSize)
            .attr("fill", 'rgba(255,55,255,0)')
            .on("click", function() {})
            .on("mouseover", function(d) {
                gear.attr("stroke", '#454545');
                gear.attr("fill", '#454545'); })
            .on("mouseout", function(d) {
                gear.attr("stroke", backgroundColor);
                gear.attr("fill", backgroundColor);})
            .on("click", function(d) {
            if (!settingsVisible) {
                hideTips();
