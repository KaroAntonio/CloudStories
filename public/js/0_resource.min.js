...
""!=generatedLine&&branches.push(r)
var i=$(".line").height(),s=Math.min(i/2,(branches.length-1)*i).toString()+"px"
if($("#branches").css("padding-bottom",s),$("#branches").empty(),branches.length>0){branches.sort(function(e,t){return parseFloat(e.visits)-parseFloat(t.visits)}),branches.reverse()
var o=[]
o[0]=branches.splice(0,1),o[1]=branches.splice(0,1*branches.length/3),o[2]=branches.splice(0,branches.length)
for(var a=0;a<o.length;a++)if(o[a].length>0){var l=Math.floor(Math.random()*o[a].length)
branches.unshift(o[a].splice(l,1)[0])}branches.length>0?selected=branches[branches.length-1]:selected=null,branches.sort(function(e,t){return parseFloat(t.visits)-parseFloat(e.visits)})
var c=backColor
for(0==branches[0].visits&&(c=maxBranchColor),branchColorScale=d3.scale.linear().range([c,maxBranchColor]).domain([0,branches[0].visits]).clamp(!0),branches.length<=3&&$("#branches").height(Math.min($(".line").height()*branches.length,$(".line").height())),a=0;a<branches.length;a++)drawLine(branches[a],branchColorScale,$("#branches"),0==a)
for(a=0;a<o.length;a++)for(l=0;l<o[a].length;l++)drawLine(o[a][l],branchColorScale,$("#branches"))}else selected=null,$("#branches").height(10)
...
"Anonymous"!=e.author_name&&void 0!=e.author_name&&(s.innerHTML="-"+e.author_name),s.id="info_"+e.id,s.className="info",debug&&(s.innerHTML+=" : "+e.visits)
var o=document.createElement("DIV")
colorLine(e,t,o,n),o.id="line_"+e.id,o.className="line",o.innerHTML=e.line
var a=function(e,t,n){return function(){t.style.color="#aaa",e.style["font-style"]="italic",e.style.color=colorScale(0)}}(o,s,e),l=function(e,r,i,s){return function(){r.style.color="transparent",e.style["font-style"]="normal",colorLine(s,t,o,n)}}(o,s,i,e)
o.onmouseenter=a,s.onmouseenter=a,o.onmouseout=l,s.onmouseout=l,o.onclick=function(e){return function(){clickStory(e)}}(e),n[0].appendChild(s),n[0].appendChild(o),s.style.marginLeft=-s.offsetWidth+"px"}function drawStats(){null!=user&&($("#user_stats")[0].innerHTML=""+user.experience+" // "+user.prestige+" // "+num_lines)}function displayAlphaWarning(){$("#alpha_warning").css("display","block")}function hideAlphaWarning(){$("#alpha_warning").css("display","none")}function setup(){var e=createCanvas(window.innerWidth,window.innerHeight)
