...
var a=backColor
for(0==branches[0].visits&&(a=maxBranchColor),branchColorScale=d3.scale.linear().range([a,maxBranchColor]).domain([0,branches[0].visits]).clamp(!0),branches.length<=3&&$("#branches").height($(".line").height()*branches.length),s=0;s<branches.length;s++)drawLine(branches[s],branchColorScale,$("#branches"),0==s)
for(s=0;s<i.length;s++)for(o=0;o<i[s].length;o++)drawLine(i[s][o],branchColorScale,$("#branches"))}else selected=null,$("#branches").height(10)
